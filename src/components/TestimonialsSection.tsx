      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(1.25rem,5vw,3.5rem)' }}>
        <ScrollReveal>
          <div style={{ maxWidth: 640, margin: '0 auto', textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-space-grotesk)', fontSize: 'clamp(1.8rem,3.5vw,2.6rem)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.15, marginBottom: '1rem', color: '#0c0a18' }}>What Clients Say About Working With Olswen</h2>
            <p style={{ color: '#4a4560', fontSize: '1.05rem', lineHeight: 1.75 }}>From active retainer clients to one-time high-stakes operations — the standard does not change.</p>
          </div>
        </ScrollReveal>
            ...
            <ScrollReveal key={i}>
              ...
              >
                <div style={{ display: 'flex', gap: 2, marginBottom: '0.5rem' }}>
                  {[...Array(5)].map((_, j) => <IconStar key={j} size={14} className="text-g400" />)}
                </div>
        ...
        <ScrollReveal>
          <div style={{ background: '#f4f1fd', border: '1px solid #c8bff2', borderRadius: 18, padding: '2rem 2.5rem', marginTop: '3rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <p style={{ fontFamily: 'var(--font-space-grotesk)', fontSize: '1.05rem', fontWeight: 600, color: '#3d2a8a', margin: 0 }}>Join Nigeria’s most trusted security client roster.</p>
            <a href="#contact" style={{ display: 'inline-flex', alignItems: 'center', fontFamily: 'var(--font-space-grotesk)', fontSize: '0.875rem', fontWeight: 700, padding: '0.85rem 2rem', borderRadius: 8, background: '#5c43c8', color: '#fff', textDecoration: 'none', whiteSpace: 'nowrap' }}>New engagements open. We respond within 24 hours.</a>
          </div>
        </ScrollReveal>
      </div>
      <style>{`
